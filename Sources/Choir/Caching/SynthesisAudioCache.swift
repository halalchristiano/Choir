...
public struct SynthesisCacheKey: Sendable, Equatable, Hashable, Codable, CustomStringConvertible {
    ...
    public init(
        ...
        self.init(
            ...
            voicePackIdentity: voicePackIdentity)
    }
    ...
    public init(
        ...
        // Appended only when a pack is in use, so every key computed before
        // voice packs existed keeps exactly the digest it had. Without this, a
        // retrained voice would be served audio cached from the previous one.
        if let voicePackIdentity {
            Self.appendComponent(Data("voice-pack".utf8), to: &canonical)
            Self.appendComponent(Data(voicePackIdentity.utf8), to: &canonical)
        }
...
/// Kept as a name so the key construction above reads unchanged; the
/// implementation is shared with voice-pack verification in `SHA256Hasher`.
