...
struct AudioSafetyTests {
    ...
    func testSoftClipFullScale() {
        ...
        #expect(clipped.allSatisfy { abs(Int($0)) < 32767 })
    ...
    func testCompressFullScale() {
        ...
        #expect(abs(Int(compressed[0])) < 32767)
    ...
    func testQualityAnalysisFullScale() {
        ...
        // The loudest excursion is negative; peak is reported as a magnitude,
        // clamped to Int16.max because 32768 is not representable.
    ...
    func testQualityReportNegativePeak() {
        ...
        #expect(report.metrics.peakDBFS.isFinite)
        #expect(abs(report.metrics.peakDBFS) < 1.0)  // full scale is 0 dBFS
    ...
    func testSampleSynthesizerFullScale() async throws {
        ...
        let peak = output.map { abs(Int($0)) }.max() ?? 0
        #expect(peak > 30_000)
        #expect(peak < 32767)
    }
}
